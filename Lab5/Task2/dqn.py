    ...
    def run(self, episodes=10000):
        for ep in range(episodes):
            ...
            if ep % 20 == 0:
                ...
                self.true_eval_count += 1
                if eval_reward > self.best_reward:
                    self.best_reward = eval_reward
                    model_path = os.path.join(self.save_dir, "best_model.pt")
                    torch.save(self.q_net.state_dict(), model_path)
                    print(f"Saved new best model to {model_path} with reward {eval_reward}")
                print(f"[TrueEval] Ep: {self.true_eval_count} Eval Reward: {eval_reward:.2f} SC: {self.env_count} UC: {self.train_count}")
                wandb.log({
                    "Env Step Count": self.env_count,
                    "Update Count": self.train_count,
                    "Eval Reward": eval_reward
                })
        print(f"Avg eval reward : {self.eval_final_reward/self.true_eval_count}")
...
if __name__ == "__main__":
    ...
    wandb.init(project="DLP-Lab5-DQN-pong-v5", name=args.wandb_run_name, save_code=True, mode="offline")
