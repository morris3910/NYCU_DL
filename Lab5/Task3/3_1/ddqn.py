...
class AtariPreprocessor:
    ...
    def reset(self, obs):
        frame = self.preprocess(obs) # preprocess the frame(RGB)
        self.frames = deque([frame for _ in range(self.frame_stack)], maxlen=self.frame_stack) # fill in the deque with same frame, cause when reseting, there's only one frame
        return np.stack(self.frames, axis=0) # fold up those frames with a new dim. and return it as a state 

    def step(self, obs):
        frame = self.preprocess(obs)
        self.frames.append(frame) # push new frame into deque
...
class DQNAgent:
    ...
    def train(self):
        ...
        with torch.no_grad():
            next_actions = self.q_net(next_states).argmax(1, keepdim=True)   # online policy
